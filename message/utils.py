...
async def print_message_to_alt_by_id(message: types.Message, id: str):
    ...
    if post['typeof'] =='hard':
        keyb = make_row_keyboard(['✏Описание', '✏Фото/файл', '✏Локация', 'Переименовать', 'Новый подпост', 'Удалить пост', '❌'])
    else:
        keyb = make_row_keyboard(['✏Описание', 'Переименовать', 'Новый подпост', 'Удалить пост', '❌'])
